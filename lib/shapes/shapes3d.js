...
class Transform3D {
    projectionTo2D () {
        ...
        Object.keys(this._normalizedPoints).forEach(key => {
            ...
            projectionResult = Matrix.dot([ this._normalizedPoints[key].asArray ], projectorMatrix);
    ...
    rotateX (deg) {
        ...
        let rotatedMatrix;
        Object.keys(this._normalizedPoints).forEach(key => {
            rotatedMatrix = Matrix.dot([ this._normalizedPoints[key].asArray ], rotaterMatrix);
    ...
    scale (scalarMultiple) {
        ...
        Object.keys(this.projectedPoints).forEach(key =>{
            scaledMatrix = Matrix.dot([ this.projectedPoints[key].asArray ], multiplerMatrix);
