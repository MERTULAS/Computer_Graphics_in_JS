...
class Box extends Transform3D {
    ...
    */
    ...
    #drawEdge(point1, point2) {
        ...
        window.__ctx__.moveTo(...point1.asArray);
        window.__ctx__.lineTo(...point2.asArray);
    ...
    draw() {
        ...
        if (this.#wireframeType === "edges") {
            edges = this.edges;
        } else if (this.#wireframeType === "triangles") {
            edges = this.triangles;
        }

        for (const edge of edges) {
            for (let i = 0; i < edge.length; i++) {
                const point1 = this.points[edge[i]];
                const point2 = this.points[edge[(i + 1) % edge.length]];
